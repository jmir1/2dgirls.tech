...
function get_episode(mal_id, anime_id, episode) {
    ...
    get_episode.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            //console.log(result);
            if(mal_id && total_eps) {
                var url = new URL(result.links[0].link);
                var link = (url.search == '') ? result.links[0].link + "?mal_id=" + mal_id + "&ep=" + episode + "&max_ep=" + total_eps : result.links[0].link + "&mal_id=" + mal_id + "&ep=" + episode + "&max_ep=" + total_eps;
                document.getElementById("ep_" + episode).innerHTML = "<a href='/watch/" + link + "'>Episode " + episode + "</a>";
            } else document.getElementById("ep_" + episode).innerHTML = "<a href='/watch/" + result.links[0].link + "'>Episode " + episode + "</a>";
        } else if (this.readyState == 4 && this.status == 400) {
